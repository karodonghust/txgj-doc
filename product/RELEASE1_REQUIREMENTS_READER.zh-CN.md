...
| 字段 | 内容 |
| --- | --- |
| 文档状态 | `reader_copy_confirmed` |
| 阅读对象 | 项目接手人、业务负责人、后续开发人员 |
| 用户确认日期 | 2026-08-17；确认现有需求方向可以作为后续工作的正式依据，未提出需求修改 |
| 当前目标 | 已完成需求方向确认；进入需求追踪矩阵和首个开发切片选择 |
...
| 3 | 第 6-7 章：案件阶段与评估 | `已确认` | 2026-08-17，未提出修改；16 字段与正式 schema 的实现差异进入追踪矩阵 |
| 4 | 第 8-10 章：学校、任务和文件 | `已确认` | 2026-08-17，未提出修改 |
| 5 | 第 11-14 章：通知、Portal、平台计数和爬虫 | `已确认` | 2026-08-17，未提出修改 |
| 6 | 第 15-17 章：质量规则、完整示例和重点问题 | `已确认` | 2026-08-17，确认作为后续工作依据 |

全部轮次已经确认，本文仍保持“阅读版”身份，不替代决策台账。正式需求变更仍需写回决策台账；当前进入“需求 -> 页面 -> API -> 模块 -> 数据库 -> 测试”的追踪矩阵，再选择首个开发切片。
