...
| Schools 模块通用设计 | `confirmed` | D4 按 EDB 编号界定学校粒度；N2 按归一化后的学年/招生类型/年级匹配及未知学年每次新建候选；年级按页面词汇归一化，不再以学校级学制判定为前置条件；枚举包含所列年级、区间包含首尾连续年级；同校不同招生记录分开保留。国际英制/美制展示分组及双语/版本页面是否构成独立 N2 记录仍为 `pending_review`，人工合并候选功能仍为 `pending_review`。首次 EDB 全量导入、逐校周期抓取（最快每天一次）、人工/爬虫学校免额外审批进入可用状态、不显示未验证标记已确认；v49 仅豁免首次 EDB 自动带入网址，后续人工来源变更仍须 Founder 审批；停用/退场机制及与 v52 手动补充的交界仍为 `pending_review`，详见 BR-051 |
