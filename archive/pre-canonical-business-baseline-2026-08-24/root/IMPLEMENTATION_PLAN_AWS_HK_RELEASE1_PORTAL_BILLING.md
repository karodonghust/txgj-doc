...
- 正常終止／重簽語義已由 `DEC-004/071` 固定：Founder 結案舊 case，重簽建立新 case，絕不重開舊 case。`DP-03` 只決定舊 case 結案後是否仍可透過 Portal 唯讀查看，以及何時 revoke grant；pending delete、viewer relationship 無效或 issuer 被停用仍須 request-time deny。此 Portal 可見性未批准前 production activation blocked。
...
`advancing_case_count` 不可用 UI 臨時計數。由 Cases module 發出不含 PII 的 case lifecycle projection event，PlatformBilling 按批准的 count policy 建立可重建 projection。`DEC-074` 已固定 count-only 映射：計入 `background_collection`、`school_selection_confirmed`、`application_in_progress`（包括暫停及全拒但未結案），排除 `signed`、整案終止待結案、`pending_delete`、`closed`；每案每個香港月末 cutoff 最多計 0 或 1，不 prorate。這不批准任何計價公式或金額。
...
| `BILLING_CUTOFF_DAY` | policy config | 固定香港曆月最後一刻；不可由 UI 任意改寫 `[DEC-074]` |
...
| `DP-03` | 舊 case 結案後是否仍可透過 Portal 唯讀查看，以及關係失效、issuer 停用、tenant past_due 時如何處理 grant？ | revoke/relationship invalid 立即 deny；case closed 是否保留只讀由 Product/Legal 決定。正常終止後重簽一律新建 case，不屬於本決策 | 改變資料可見性與 retention；不改變案件終止／重簽規則 |
...
| `DP-06` | 「正在推進的 case」及月度計價公式 | **Count-only 映射已由 `DEC-074` 批准：**計入三個進行中里程碑，含暫停／全拒未結案；排除 signed／整案終止待結案／pending_delete／closed；香港月末 cutoff、每案 0/1、不 prorate。計價公式仍未批准 | 可實作版本化 count projection；仍不能生成金額 |
