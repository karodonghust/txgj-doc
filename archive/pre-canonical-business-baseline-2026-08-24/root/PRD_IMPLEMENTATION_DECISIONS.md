...
| --- | --- |
| 文件版本 | v0.19 |
| 文件狀態 | `DEC-071` K12 案件流程已採納；`DEC-073` 恢復 Release 1 只提供站內通知的舊規則；`DEC-074` 已收口通知、Assessment 與推進中案件計數；`DEC-075` 補充 Student 性別資料規則；受影響實作需重新規劃，外部副作用與上線操作仍需逐項批准 |
| 任務 ID | `prd-implementation-design-2026-07-31` |
| 最後更新 | 2026-08-24（Asia/Singapore） |
| 當前目標 | 把 `PRD.md` 轉成可實施、可驗證、可遷移和可回滾的生產級方案 |
| 當前階段 | 2026-08-24 已採納 `DEC-071` 業務流程，由 `DEC-073` 確認 Release 1 不發送外部業務 Email，並由 `DEC-074` 收口通知、Assessment 與推進中案件計數；K12 案件流程確認完成。公共與業務模組逐項審閱期間，`DEC-075` 新增 Student 性別資料要求；既有實作需進入差異規劃。既有 `P3-02` evidence 保持歷史 `needs_human`，計價、`DP-01`–`DP-12` 其餘執行 gate 與 `DEC-060` 仍 fail closed；不授權 Terraform plan/apply、production/cloud/data action |
...
| `amended` | 決策仍有部分成立，但部分語義或技術載體已被新條件修訂；必須同時閱讀後續決策 |
...
- 包含：身份與帳號生命週期、RBAC/案件授權、審計、Student/Guardian/ServiceCase/SchoolTarget/Task、受控狀態、看板、文件、站內通知、受限單一案件只讀 Portal，以及 aggregate-only PlatformBilling／合同版本／月度收費通知草稿。`[DEC-064-066,073]`
- 不包含：Portal 寫入／公開註冊／文件下載、正式稅務發票、付款、退款、會計總帳、收入確認、向 Guardian 或 Student 發送的外部業務 Email、SMS／WhatsApp 自動通知、自動快照同步、Excel 匯入和 AI 對客報告。`DP-01`–`DP-12` 與外部通知一律 fail closed。
...
- 狀態：`resolved`，由 `DEC-023`、`DEC-032`、`DEC-033`、`DEC-039`、`DEC-073`、`DEC-074` 鎖定資料、通知、保留、提醒節奏和副作用邊界；營運 metric/alert catalogue 由已解決的 `OD-11` 管理。
- 已批准：log/metric/trace 欄位白名單、30 天一般 log、1 年 audit、PII redaction 和香港落點；Release 1 只提供站內通知，不向 Guardian 或 Student 發送外部業務 Email。
- 已批准：站內通知只含「有待辦事項」，不含 Student/Guardian/Case/SchoolTarget/文件細節；精確觸發、收件人、到期前提醒和每日逾期節奏見 `DEC-074`。Email、SMS 和 WhatsApp 業務通知均排除。
...
2. `partially_resolved`：`DEC-071` 已鎖定案件/逐校狀態、選校確認、暫停/恢復、人工結案、終止服務、重新簽約和確認後改校的業務規則。暫停原因在 Release 1 只用必填自由文字，不設固定分類。客戶終止整體服務時，進行中 SchoolTarget 記為 `withdrawn`，相關未完成 Task 取消但保留歷史，再由 Founder 人工結案並保存原因；日後重新簽約一律建立新 ServiceCase，不重開舊案。確認後改校建立新名單版本並重新經 Founder/Guardian 確認；不變 SchoolTarget/Task 原樣保留，新增學校確認後才建立申請 Task，仍在進行中的移除學校記為 `withdrawn` 並取消其未完成 Task，已終態 SchoolTarget 保留原結果不得改寫。暫停資料表示和精確實施 contract 仍需設計，不再是客戶業務問題。
3. `partially_resolved`：`DEC-071` 已鎖定逐校核心語義、可選面試、Guardian offer 決定、Primary Advisor 代錄渠道、逐校材料清單／提交證據、下載角色，以及 Advisor／Contractor 面試輔助人的最小資料權限；精確 DDL、API、授權、audit 和合法轉移屬實作差異設計，不再是客戶業務問題。
4. `partially_resolved`：`DEC-071` 已鎖定兩類自動 Task、背景收集不建 Task、Primary Advisor 可人工建立臨時 Task，以及 `DEC-028` 的接受／拒絕／同 Task 重派／完成／逾期提示規則；Founder 不逐項驗收 Task。舊 policy、schema、API 和 UI 仍需前向修訂，不再等待客戶決定 actor matrix。
5. `resolved`：`DEC-029` 已鎖定期限/revoke；`OD-07` 已確認 Release 1 不提供批量授權，僅允許 Founder 在 Primary Advisor 停用或缺席達 24 小時時簽發受限、最長 4 小時的 `case_summary:view` break-glass grant。這不再是客戶待確認項。
...
- 狀態：`amended`（案件與逐校狀態分離的原則仍成立；八階段主狀態及舊逐校結果語義由 `DEC-071` 修訂）
...
- 2026-08-24 起，第一點中的八階段和逐校終態集合不得再作現行實作前提；現行完整語義見 `DEC-071` 與 `product/K12_CASE_AND_SCHOOL_APPLICATION_WORKFLOW.zh-CN.md`。

### DEC-028：Task 使用獨立受控狀態機

- 狀態：`amended`（2026-08-24 使用者確認新 K12 Task 流轉；原 `reassigned`／`approved`／`overdue` 持久狀態解讀不再是現行業務要求）
- 新 Task 從 `assigned` 開始；當前 Assignee 可接受或填寫原因拒絕。拒絕結束當前 TaskAssignment，Task 本身保留並等待 Primary Advisor 重派；換人時同樣在原 Task 上建立新 Assignment、保留全部歷史，新 Assignee 回到待接受，不建立替代 Task。
- `overdue` 由未完成 Task 的 `due_at` 計算為提示／提醒標記，不是阻斷操作的獨立業務狀態；逾期後仍可接受、重新分派或完成。
- Assignee 保存該 Task 類型要求的完成記錄和證據後即可完成；Release 1 不要求 Founder 逐條驗收，也不再以 `approved` 作完成後的必經狀態。
- Primary Advisor 可為自己負責的案件人工建立真實需要的臨時 Task，但人工 Task 不自動推進 ServiceCase 或 SchoolTarget。自動建立仍僅限 `DEC-071` 的逐校申請與必要面試兩類。
- 只有 Assignee 或具明確權限的 actor 可操作；拒絕、重新分派、取消和完成都保存操作者、時間、原因或完成記錄與 audit，不覆蓋歷史。
...
- 狀態：`accepted`（2026-08-24 由 `DEC-073` 再次確認）
- Release 1 禁止發送任何外部業務 Email、WhatsApp 或 SMS；Guardian 和 Student 都不接收平台外部業務通知。身份邀請的香港通信流程仍需另行受控處理，不能等同一般通知功能。
...
- 狀態：`amended`
- `Public Plane` 承載無 PII／無個人化 cache 的公司官網、產品資訊和獲客入口。公開官網有助於獲客但不是第二個租戶的技術前置條件。Release 1 不管理簽約前 Lead、諮詢或 Quote，不新增銷售 Lead／Quote／Contract 實體；若未來建立公開表單，Lead／Consent 必須作為另行批准的獨立域，且不得由表單直接建立正式 membership、Student 或 ServiceCase。
...
| Task | Task、assignment、accept/reject/reassign/complete/cancel、計算型 overdue、完成證據和 DDL；不擁有逐 Task Founder approve |
...
- 狀態：`amended`（案例保留與知識分離仍成立；SchoolTarget outcome 的終態及結案解讀由 `DEC-071` 修訂）
...
- 2026-08-24 起，`waitlisted` 和 Guardian 尚未決定的 `accepted` 可保存 outcome fact，但不是 terminal state；只有 `offer_confirmed`、`offer_declined`、`rejected`、`withdrawn` 可滿足逐校終態。所有終態與無 open Task 仍只是人工結案前提，不得自動結案。`[DEC-071]`
...
### DEC-071：K12 案件與逐校申請流程修訂

- 狀態：`accepted`（2026-08-24 使用者確認；只批准業務需求基線，不授權程式、migration、資料、雲端、commit、push 或 deployment）
- 本決策修訂 `DEC-027` 的案件主狀態與逐校狀態集合，也修訂 `DEC-058` 對候補、錄取和結案 guard 的解讀；兩者其餘不衝突原則仍有效。
- `ServiceCase` 只表達整體里程碑：`signed -> background_collection -> school_selection_confirmed -> application_in_progress -> closed`。逐校的準備、提交、面試、候補和結果只由 `SchoolTarget` 表達；舊八階段線性主流程不再是現行業務要求。
- `signed` 保存簽約事實／起始里程碑；建立案件並指定 Primary Advisor 後自動進入 `background_collection`，不要求人工階段命令。Assessment 從 `draft` 開始；背景 blockers 完成後記錄 `background_complete` 並允許建立候選學校名單，而不是此時才進入背景收集階段。
- Release 1 從客戶已在線下完成簽約開始，不管理簽約前 Lead、諮詢或 Quote，也不新增銷售 Lead／Quote／Contract 實體。簽約後才建立或關聯 Student、Guardian 和 ServiceCase；已簽合同文件如需保存，復用案件 Document 並關聯 ServiceCase。此邊界不影響 Platform Billing 的合同參考版本。
- 進行中的案件必須可因家長要求暫緩等原因標記為暫停。僅當案件尚無 SchoolTarget 達到 `submitted` 或後續狀態時，才允許案件級暫停；只要已有一所學校正式提交，就不得暫停整個案件，申請方不再繼續某校時須按該校記錄 `withdrawn`。Primary Advisor 可直接暫停或恢復自己負責的案件，無需 Founder 事前審批；Founder 可暫停或恢復任何案件。Guardian 只可提出暫停或恢復請求，由有權員工執行系統操作。每次暫停必填簡短自由文字原因，並保存操作者與操作時間；Release 1 不設固定原因分類。暫停不是結案，不覆寫暫停前里程碑，且暫停期間不得按正常流程繼續推進。恢復後案件回到暫停前里程碑，既有未完成 Task 繼續使用同一記錄，不自動取消、完成或重建。學校規定的外部申請截止日期和內部 Task 截止時間在暫停期間都繼續計算，不得自動順延；Task 可進入逾期，內部逾期提醒在暫停期間繼續發給案件 Primary Advisor、Founder 和 Task 實際 assignee，不發給 Guardian。暫停的資料表示仍待實施設計，不因此預設新增實體。
- 選校順序固定為：Primary Advisor 建立候選名單 -> Founder 批准或駁回修改 -> Guardian 確認 Founder 已批准的同一版本並保存確認記錄 -> 進入申請處理。確認後修改名單必須重新經 Founder 批准和 Guardian 確認。
- Release 1 的 Guardian 確認採員工代錄：案件 Primary Advisor 通過平台外人工電話、微信或面談取得 Guardian 對最終學校名單或 offer 決定的確認後，在系統保存確認結果、確認時間、確認方式和操作人，並關聯不可歧義的名單版本或 SchoolTarget／offer。若存在書面截圖或文件則關聯到案件；書面附件不是完成確認的強制條件。Guardian Portal 保持只讀，不承擔確認寫入；Email 已由 `DEC-073` 排除。實施優先復用案件／SchoolTarget、版本、文件與 audit 能力，不因代錄流程自動新增實體。
- 已確認名單發生任何變更時，建立新名單版本並重新經 Founder 批准和 Guardian 確認。不變的 SchoolTarget 及既有 Task 原樣保留，不重建；新增學校只有在新版確認後才進入 `preparing` 並建立申請 Task；仍在進行中的移除學校記為 `withdrawn`，其未完成 Task 取消但保留完整歷史；已進入終態的 SchoolTarget 保留原結果，不得改寫為 `withdrawn`。舊名單、SchoolTarget 和 Task 均不得刪除或覆蓋。
- `waitlisted` 與學校已錄取但 Guardian 尚未決定的 `accepted` 都不是終態。Guardian 接受或拒絕 offer 分別保存為 `offer_confirmed`、`offer_declined`；逐校終態為 `offer_confirmed`、`offer_declined`、`rejected`、`withdrawn`。
- 每個已確認 SchoolTarget 進入 `preparing` 時，逐校指派 Primary Advisor 或具明確案件授權的 Case Collaborator 作申請負責人，並自動建立一條「準備並提交申請」Task。學校要求面試時，自動建立一條面試輔助 Task；預設由 Primary Advisor 負責，也可指派已作為 CaseCollaborator 的正式 Advisor，或只透過該 TaskAssignment 取得脫敏工作區的 Contractor。背景資料收集不建立 Task。
- 非 Primary Advisor 的面試輔助人只可查看該 Task、目標學校、面試時間／方式／語言、輔導要求和 Primary Advisor 整理的必要背景摘要；不得查看完整 Assessment、Guardian／聯絡資料、內部備註、案件文件、download／export、其他學校或其他案件。Contractor 不因此成為 CaseCollaborator、申請提交負責人或案件成員；拒絕 Task，或 Task 完成、取消、重派、案件結束即失去該工作區存取。
- Release 1 不設全校通用固定材料清單；每個 SchoolTarget 按該校當期官方要求維護自己的清單。申請 Task 只有在保存提交時間、渠道、提交人、清單完成狀態，以及學校參考號或至少一份其他提交憑證後才能完成並把 SchoolTarget 推進至 `submitted`。學校未提供參考號時須明確記錄並保存其他憑證。
- 提交憑證復用 SchoolTarget、Task、Case Document 和 audit／version 能力，不新增 Material 或 SubmissionEvidence 實體。Founder、案件當前 Primary Advisor 和該校當前 Application Assignee 可下載本次申請必需的已掃描可用文件；其他未授權角色、僅負責面試輔助者、Guardian、Student 和 Portal 不可下載。Application Assignee 不再擔任該校負責人或既有案件授權失效後不得繼續下載。
- 所有現有學校均終止且沒有未完成 Task 只是結案前提，不是自動觸發。業務必須允許新增學校，或由 Founder 明確選擇結案並保存結果與理由；`waitlisted` 或 `accepted` 尚未完成 Guardian 決定時不得結案。
- 客戶要求終止整體服務時，進行中的 SchoolTarget 均記為 `withdrawn`，相關未完成 Task 取消但保留歷史；完成終止處理後由 Founder 人工結案並保存原因，不得自動結案。客戶日後重新簽約必須建立新 ServiceCase，不得重開或覆寫舊案。
- 不因確認記錄、名單版本或「需要決定下一步」提示自動新增實體或持久化狀態；先證明現有聚合、版本與 audit 無法滿足要求，再單獨評審新實體。
- 完整流程、狀態語義、Task 觸發、流程圖、已確認規則和實施影響見 `product/K12_CASE_AND_SCHOOL_APPLICATION_WORKFLOW.zh-CN.md`。現有實作及票據若與本決策衝突，均需先重新規劃和驗證。

### DEC-072：Release 1 Guardian Email 收件人邊界

- 狀態：`superseded`（2026-08-24 曾暫定 Guardian-only Email；同日由使用者明確選擇遵循舊規則，故被 `DEC-073` 取代）
- 歷史內容：曾提出 Release 1 向 Guardian 發送業務通知 Email、但不向 Student 發送。此內容不得再作現行需求或開發前提。

### DEC-073：恢復 Release 1 只提供站內通知

- 狀態：`accepted`（2026-08-24 使用者確認「遵循舊規則」；只確認業務範圍，不授權程式、migration、資料、雲端、commit、push 或 deployment）
- Release 1 遵循 `DEC-033`：平台只提供站內通知，不向 Guardian 或 Student 發送外部業務 Email，也不發送 SMS 或 WhatsApp。
- Guardian 的名單確認和 offer 決定仍必須保存確認記錄；案件 Primary Advisor 通過平台外人工電話、微信或面談取得確認後代錄，Email 和只讀 Portal 都不承擔 Release 1 的確認寫入。未來新增其他確認渠道時才需另行確認。
- 若未來需要外部 Email，必須作為新範圍重新確認觸發、收件人、內容、同意／退訂、供應商、香港處理位置、DPA、redaction、retention、失敗處理和 delivery contract，不能由開發自行恢復 `DEC-072`。

### DEC-074：K12 剩餘業務規則收口

- 狀態：`accepted`（2026-08-24 使用者逐項確認站內通知、正式 Assessment 和推進中案件計數；只確認業務規則，不授權程式、migration、資料、雲端、commit、push 或 deployment）
- 站內通知觸發固定為：Task 分配／重派通知新 Assignee；Task 被拒絕通知案件 Primary Advisor；候選名單待審批通知 Founder；Founder 批准／驳回及待 Guardian 確認通知 Primary Advisor；所有 SchoolTarget 已終態且無未完成 Task 時通知 Primary Advisor 與 Founder 選擇新增學校或人工結案。Task 到期前 3 天和 1 天通知實際 Assignee 與 Primary Advisor；逾期後每天通知實際 Assignee、Primary Advisor 與 Founder，直至 Task 完成或取消。暫停期間不停止截止計算或提醒；同一使用者、同一 effect、同一天只產生一條。通知仍只表達「有待辦事項」，不含 Student、Guardian、Case、SchoolTarget、學校或文件細節，也不發給 Guardian／Student。
- 正式 K12 Assessment 採現有已批准不可變 manifest 的 15 欄，而不是舊 16 欄展示：`student_profile` 3 欄、`education_profile` 3 欄、`school_preferences` 5 欄、`family_context` 4 欄；精確 field ID、type、enum 與兩個 blocker 集合以 `schema/k12/*.v1.json` 的批准 receipt 為準並同步到需求閱讀版／流程文檔。Primary Advisor 可讀寫；有 `education_profile:edit` 明確 grant 的 CaseCollaborator 可讀寫，有 view grant 時只讀；Founder 與 Admin 只讀；其他 Advisor、Contractor、Data Reviewer、Guardian、Student、Portal 預設不可見。`unknown` 和 `declined_to_provide` 不滿足 blocker；`not_applicable` 只有在 manifest 對該欄明確允許時才滿足，當前 v1 未允許任何 blocker 以此通過。
- `advancing_case_count_v1` 計入 `background_collection`、`school_selection_confirmed`、`application_in_progress`；案件暫停以及所有既有學校均拒絕但尚未結案時仍計入。排除瞬時 `signed`、已記錄整案終止但尚待 Founder 完成結案、`pending_delete` 和 `closed`。每案在香港月末 cutoff 最多計 0 或 1，不按學校數或天數折算、不 prorate，也不生成應付金額；遲到／修正事件只可用新 policy/snapshot version 重建。
- 至此 `product/K12_CASE_AND_SCHOOL_APPLICATION_WORKFLOW.zh-CN.md` 所列客戶業務問題已全部確認。資料表示、API、ACL、DDL、migration、舊資料兼容、通知 worker、測試和上線 gate 均屬後續差異設計與驗證，不得把本決策解讀為已實現。

### DEC-075：Student 性別資料與申請前校驗

- 狀態：`accepted`（2026-08-24 使用者在 CRM 模組審閱中確認；只確認業務需求，不授權程式、migration、資料、雲端、commit、push 或 deployment）
- `Student` 基礎資料新增可空的 `gender`，固定值為 `male`、`female`、`other`、`not_disclosed`。`null` 表示尚未收集，`not_disclosed` 表示學生或 Guardian 已明確選擇不提供；兩者不得混用。
- `gender` 屬於 CRM Student profile，不是 Assessment answer，不得在正式 15 欄 Assessment 中重複保存。它不是身份鍵、唯一條件或自動合併依據，也不得由姓名、學校、相片或其他資料推斷。
- CRM 建檔時可不填。當某一目標學校或其申請表要求性別時，進入該校申請準備前必須由有權員工補齊可用值；`null` 或 `not_disclosed` 不能被系統偽造為已滿足。`other` 若無法直接映射到該校選項，必須人工處理，不能靜默改寫為 `male` 或 `female`。
- 學校的 `gender_policy` 可產生候選資格提示或人工審查項，但不得僅憑此欄自動移除 SchoolTarget、自動拒絕申請或繞過 Founder 與 Guardian 的選校確認。
- 現有 `crm_students`、Student 建檔／編輯 API、頁面和測試均未包含此欄。後續須以追加 migration、舊資料 `null` 兼容、DTO／驗證／授權／審計 redaction 和合成測試完成差異實作；本決策不改寫既有本地驗收證據。
