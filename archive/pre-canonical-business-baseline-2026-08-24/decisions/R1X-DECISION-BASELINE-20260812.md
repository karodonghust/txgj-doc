...
| Date | 2026-08-12 (Asia/Hong_Kong); amended 2026-08-24 (Asia/Singapore) |
| Status | `implementation_baseline_selected_amended`; DP-06 count-only state mapping was replaced and confirmed on 2026-08-24; human gates below remain binding |
| Scope | `DP-01` through `DP-12` for Release 1 bounded External Portal and aggregate-only PlatformBilling |
| Authority | `DEC-060`, `DEC-064`, `DEC-065`, `DEC-066`, `DEC-071`, `DEC-074`; current implementation/production plans and existing module contracts/tests |
...
> **2026-08-24 amendment:** `DEC-071` removes four states used by the original DP-06 advancing-case mapping. `DEC-074` now approves the replacement count-only/no-charge mapping. Pricing, payable amount, notice generation and all real enablement gates remain unapproved and fail closed. Other DP baselines are unchanged by this amendment.
...
**Approval gate.** Product and Legal must approve any future closed-case visibility. Normal service termination and later re-sign follow `DEC-004/071`: Founder closes the old case and a re-sign creates a new ServiceCase; the old case is never reopened. Any future exceptional reopen proposal requires separate Founder/Product/Legal approval. DP-09 gates subscription effects.
...
**Historical selected baseline, no longer current.** The 2026-08-12 mapping counted `background_collection`, `school_selection_confirmed`, `interview_preparation`, `application_submitted`, `awaiting_result`, or `offer_confirmed`, and excluded `signed`, `closed`, `pending_delete`, plus unknown pause/cancel states.

**Current amended baseline.** Release 1 may implement a versioned **count-only** monthly metric, not a charge formula. `advancing_case_count_v1` includes `background_collection`, `school_selection_confirmed`, and `application_in_progress`, including paused cases and cases whose existing schools are all rejected but which are not yet closed. It excludes transient `signed`, a recorded whole-service termination awaiting Founder close, `pending_delete`, and `closed`. Cutoff is the last instant of the calendar month in `Asia/Hong_Kong`; one case contributes at most 0 or 1; there is no proration and no amount is generated from the count. Late or corrected events create a new policy/snapshot version rather than silently rewriting an approved snapshot. `[DEC-074]`

**Rationale.** The count must be reconstructable from an approved case-state policy without inventing price, partial-month, pause, cancellation or revenue semantics. Until that policy is approved for the amended workflow, failing closed means producing no closed count snapshot.
...
**Deterministic evidence.** One test per included/excluded state and overlay, including pause, all-rejected-not-closed and whole-service termination; cutoff before/at/after boundaries and HK timezone; duplicate/out-of-order/late events; rebuild equivalence; unknown-state and incomplete-checkpoint denial; PII-schema rejection.
